...
export default function EditAttribute() {
  ...
  useEffect(() => {
    ...
    // Check if user is authenticated and fetch attribute data
    const init = async () => {
      try {
        // Check authentication
        const authResponse = await fetch('/api/auth/session/edge', {
          method: 'GET',
          credentials: 'include'
        });
        ...
        const attrResponse = await fetch(`/api/admin/attributes/${attributeId}`, {
          method: 'GET',
          credentials: 'include'
        });
        ...
        if (!attrData.success || !attrData.attribute) {
          throw new Error('Attribute not found');
        }
        
        // Check if the attribute type matches the URL type
        if (attrData.attribute.type !== attributeType) {
        ...
        if (error.message === 'Authentication failed' || error.message === 'Admin access required') {
          router.push('/admin/login');
        } else {
          setErrorMessage(error.message || 'Failed to load attribute');
          setTimeout(() => {
            router.push('/admin/attributes');
          }, 2000);
        }
    ...
    init();
  }, [router, attributeType, attributeId, validTypes]);
  ...
  const handleSubmit = async (data: any) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(responseData.error || 'Failed to update attribute');
      }
      ...
      setErrorMessage(error.message || 'Failed to update attribute');
    ...
    <AdminPanelLayout>
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold">Edit {getAttributeTypeDisplayName()}</h1>
          <Link
